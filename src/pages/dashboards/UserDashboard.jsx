...
import { Heart, Target, PlusCircle, Clock, TrendingUp, User, LayoutDashboard, LogOut, Wallet, CheckCircle } from 'lucide-react'
...
export default function UserDashboard() {
  const { user } = useAuth()
  const { campaigns, transactions, stats } = useData()
    ...
    <div className="min-h-screen bg-[#F9F6EF]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="bg-white border border-gray-200 rounded-3xl p-6 sm:p-8 mb-8">
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5 text-gray-900">
            <div className="w-16 h-16 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-[#02a95c] font-display font-bold text-2xl">
              {user.avatar}
            </div>
          ...
          <div className="grid grid-cols-3 gap-8 mt-8 pt-8 border-t border-gray-100">
            ...
            <div><p className="text-2xl font-black text-gray-900">{myCampaigns.length}</p><p className="text-xs text-gray-400 font-bold uppercase tracking-wider">Impact</p></div>
          </div>
        </div>
      ...
      {showCreate && (
        <div className="modal-overlay">
          <div className="modal-box max-w-lg" style={{ borderRadius: '24px', alignSelf:'center' }}>
            <h2 className="font-display font-bold text-2xl text-gray-900 mb-5">Create Campaign</h2>
